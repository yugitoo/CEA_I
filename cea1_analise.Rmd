...
#### Estagio 1 - Modelo reduzido (sem interacao)
```{r}
# sem aplocar log
est$estacoes = relevel(est$estacoes, ref = "inverno") # define a casela de referencia
...
### estagio 2 - trv equivalente a anova
```{r}
lrtest(fit2_reduzido,fit2_inverno)
lrtest(fit2_reduzido,fit2_invernol)
```
...
# aplicando o log nos hormonios
fit3_reduzidol2 <- glmmTMB(Estagio.III ~ inv_prim+log(E2+1)+log(P4) +(1 | femea),
                         data=na.omit(est), 
                         family=nbinom1(link="log"))
...
lrtest(fit3_completol, fit3_completo0l)
...
```
#### Estagio 4 - trv equivalente a anova
```{r}
lrtest(fit4_completo,fit4_completo0)
lrtest(fit4_completol,fit4_completo0l)
```
...
# aplicando log
lrtest(fit4_reduzidol, fit4_completo0l)
```
